...
const createRequest = asyncHandler(async (req, res) => {
  ...
  const request = await prisma.$transaction(async (tx) => {
    ...
    await tx.requestStatusHistory.create({
      data: { requestId: created.id, fromStatus: null, toStatus: 'pending', actorUserId: req.user.id, actorType: 'resident', actorStaffId: null, note: 'Request submitted' },
    });
    await tx.auditEvent.create({
      data: auditData(req, {
        module: 'requests', action: 'created', recordId: created.id,
        recordLabel: created.trackingNumber || created.documentType,
        after: { status: created.status, documentType: created.documentType, purpose: created.purpose },
...
const confirmClaimed = asyncHandler(async (req, res) => {
  ...
  if (!isValidRequestTransition('resident', existing.status, 'completed')) {
    return res.status(400).json({ success: false, message: `This request cannot be confirmed claimed from its current status (${existing.status}).` });
  }
  const [request] = await prisma.$transaction([
    prisma.documentRequest.update({ where: { id: existing.id }, data: { status: 'completed' } }),
    prisma.requestStatusHistory.create({
      data: { requestId: existing.id, fromStatus: existing.status, toStatus: 'completed', actorUserId: req.user.id, actorType: 'resident', actorStaffId: null, note: 'Resident confirmed claim' },
    }),
    prisma.auditEvent.create({
      data: auditData(req, {
        module: 'requests', action: 'status_changed', recordId: existing.id,
        recordLabel: existing.trackingNumber || existing.documentType,
        before: { status: existing.status }, after: { status: 'completed' }, note: 'Resident confirmed claim',
      }),
    }),
  ]);
  res.json({ success: true, request });
